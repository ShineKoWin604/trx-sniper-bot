...
# ဇ အရမ်းမတက်စေရန် ၄ ဆင့်သာ ကန့်သတ်ထားပါသည်
X_MULTIPLIERS = [1, 3, 9, 27] 

class KyaltaryarBigSmallAssemblyV18:
    def __init__(self):
        self.last_period = None
        self.last_pred_side = None # "Big" သို့မဟုတ် "Small"
        ...
        self.last_failed_sides = [] # အမှားပတ်လမ်းကို စောင့်ကြည့်ရန်

        # --- 🔮 Power Numbers Mapping (ပုံ - ၁ အတိုင်း) ---
        self.power_numbers = {
            1: 4, 2: 5, 3: 6, 4: 7, 5: 8,
            6: 9, 7: 0, 8: 1, 9: 2, 0: 3
        }
    ...
    # --- 🤖 AI BRAINS RE-ENGINEERED WITH IMAGE TRICKS ---

    def chatgpt_brain(self, history):
        # Trick: ပုံ ၄ အတိုင်း ၃ သို့မဟုတ် ၇ ထွက်ခဲ့ရင် နောက်တစ်ခု Big လို့ ယူဆမယ်
        if len(history) >= 1:
            last = history[-1]
            if last in [3, 7]:
                return "Big", self.power_numbers.get(last, 7)
            if last in [4, 6]:
                return "Small", self.power_numbers.get(last, 2)
    ...
    def deepseek_brain(self, history):
        # Trick: ပုံ ၄ အတိုင်း 0 နှစ်ခါဆင့်ထွက်ရင် Big
    ...
    def gemini_brain(self, history):
        # Trick: Power Number Matrix အရ ခန့်မှန်းခြင်း
        if history:
            last = history[-1]
    ...
    def claude_brain(self, history):
        # Trick: ပုံ ၄ ပါ ၁ နှင့် ၇ တွဲထွက်ပါက Big
        if len(history) >= 2 and ((history[-2] == 1 and history[-1] == 7) or (history[-2] == 7 and history[-1] == 1)):
            return "Big", 7
        ...
        if big_count > small_count:
            return "Big", max([x for x in recent if x >= 5], default=7)
        else:
            return "Small", max([x for x in recent if x < 5], default=2)

    def grok_brain(self, history):
        # Trick: ပုံ ၄ အရ ၉ နှင့် ၆ Pattern အတွက် Small ဟုသတ်မှတ်
        if len(history) >= 1 and history[-1] in [9, 6]:
            return "Small", self.power_numbers.get(history[-1], 2)
    ...
    def llama_brain(self, history):
        # Chart Flow (ပုံ ၃ မှ Pattern အချို့)
        if len(history) >= 2:
            # 3, 8 ထွက်ပြီးရင် Big လို့ Chart မှာ အတွေ့ရများ
            if history[-2] == 3 and history[-1] == 8: return "Big", 9
            # 0, 5 ထွက်ပြီးရင် Small လို့ Chart မှာပြထား
    ...
    def mistral_brain(self, history):
        # Trick: ပုံ ၂ ပါ Trend များကို အခြေခံခြင်း
        if len(history) >= 1 and history[-1] == 3:
            return "Big", 6
    ...
    # --- 🏛️ SOVEREIGN CONFERENCE ASSEMBLY FOR BIG/SMALL ---
    ...
    def execute_big_small_conference(self, num_history):
        ...
        ai_reports = [
            self.chatgpt_brain(num_history),
            self.deepseek_brain(num_history),
            self.gemini_brain(num_history),
            self.claude_brain(num_history),
            self.grok_brain(num_history),
            self.llama_brain(num_history),
            self.qwen_brain(num_history),
            self.mistral_brain(num_history),
            self.phi_brain(num_history)
        ]

        # Big သို့မဟုတ် Small မဲခွဲခြင်း
        votes = {"Big": 0, "Small": 0}
            ...
            if bot.last_period:
                ...
                if bot.last_pred_side is not None:
                    ...
                    if is_win:
                        ...
                        if len(bot.last_failed_sides) > 2:
                            bot.last_failed_sides.pop(0)
                            
                        current_step = bot.users.get(str(MY_ID), 0)
                        bot.users[str(MY_ID)] = min(current_step + 1, len(X_MULTIPLIERS) - 1)

            # Big/Small အဖြစ် အစည်းအဝေးပွဲမှ ခန့်မှန်းခြင်း
            p_side, p_num, p_conf = bot.execute_big_small_conference(num_history)
            bot.last_pred_side = p_side
            bot.last_pred_num = p_num  
            bot.current_prob = p_conf
